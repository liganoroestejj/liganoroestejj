...
const faqs = [
  { q: "Quando acontecem os campeonatos?", a: "Os campeonatos são realizados ao longo do ano, com 8 etapas entre maio e novembro." },
  { q: "Como acessar o ranking?",           a: "O ranking é atualizado após cada etapa e pode ser acessado na seção Ranking." },
  ...
  return (
    <section style={{ padding: "32px 24px", background: "#F9F9F7" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
        ...
        <span style={{ color: "#F0B90B", fontSize: 8, fontWeight: 700, letterSpacing: 4, textTransform: "uppercase" }}>Dúvidas</span>
      </div>
      <h2 style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 32, color: "#0A0A0A", letterSpacing: 1, marginBottom: 20 }}>Perguntas Frequentes</h2>
      {faqs.map((f, i) => (
        <div key={i} style={{ borderBottom: "1px solid #eee", borderTop: i === 0 ? "1px solid #eee" : "none" }}>
          <button onClick={() => setOpen(open === i ? null : i)} style={{ width: "100%", padding: "14px 0", display: "flex", justifyContent: "space-between", alignItems: "center", background: "none", border: "none", cursor: "pointer" }}>
            <span style={{ fontSize: 10, fontWeight: 700, color: "#0A0A0A", textTransform: "uppercase", textAlign: "left", paddingRight: 10 }}>{f.q}</span>
          ...
          {open === i && <p style={{ fontSize: 11, color: "#666", lineHeight: 1.7, paddingBottom: 14 }}>{f.a}</p>}
        </div>
      ))}
    </section>
  )
