...
const faqs = [
  { q: "Quando acontecem os campeonatos de 2025?", a: "Os campeonatos são realizados ao longo do ano, com 8 etapas distribuídas entre maio e novembro. Confira o calendário completo na seção Calendário." },
  ...
  return (
    <section style={{ padding: "64px 60px", background: "#F9F9F7" }}>
      <div style={{ display: "inline-flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
        ...
        <span style={{ color: "#F0B90B", fontSize: 10, fontWeight: 700, letterSpacing: 5, textTransform: "uppercase" }}>Dúvidas</span>
      </div>
      <h2 style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 44, color: "#0A0A0A", letterSpacing: 1, lineHeight: 1, marginBottom: 32 }}>Perguntas Frequentes</h2>
      {faqs.map((f, i) => (
        <div key={i} style={{ borderBottom: "1px solid #eee", borderTop: i === 0 ? "1px solid #eee" : "none" }}>
          <button onClick={() => setOpen(open === i ? null : i)} style={{ width: "100%", padding: "18px 0", display: "flex", justifyContent: "space-between", alignItems: "center", background: "none", border: "none", cursor: "pointer" }}>
            <span style={{ fontSize: 11, fontWeight: 700, color: "#0A0A0A", letterSpacing: 0.5, textTransform: "uppercase", textAlign: "left" }}>{f.q}</span>
          ...
          {open === i && <p style={{ fontSize: 11, color: "#666", lineHeight: 1.7, paddingBottom: 18 }}>{f.a}</p>}
        </div>
      ))}
    </section>
  )
