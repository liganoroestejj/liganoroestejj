const features = [
  ...
  { title: "Circuito Completo", desc: "Etapas e eventos Opens ao longo do ano"},
]
  ...
  return (
    <section style={{ padding: "64px 60px", background: "#F9F9F7" }}>
      <div style={{ display: "inline-flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
        ...
        <span style={{ color: "#F0B90B", fontSize: 12, fontWeight: 700, letterSpacing: 5, textTransform: "uppercase" }}>Por que se filiar</span>
      </div>
      <h2 style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 46, color: "#0A0A0A", letterSpacing: 1, lineHeight: 1, marginBottom: 32 }}>Vantagens da Liga</h2>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 1, background: "#eee", borderRadius: 12, overflow: "hidden", border: "1px solid #eee" }}>
        {features.map((f, i) => (
          <div key={i} style={{ background: "#fff", padding: "32px 24px", textAlign: "center" }}>
            ...
            <div style={{ fontSize: 16, fontWeight: 800, color: "#0A0A0A", marginBottom: 7 }}>{f.title}</div>
            <div style={{ fontSize: 13, color: "#999", lineHeight: 1.6 }}>{f.desc}</div>
          </div>
        ))}
      </div>
    </section>
