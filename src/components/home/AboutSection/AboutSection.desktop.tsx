const metrics = [
  { n: "50",  sym: "+", label: "Atletas Filiados" },
  { n: "15",  sym: "+", label: "Academias Parceiras" },
  { n: "6",   sym: "×", label: "Etapas por Temporada" },
]
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 16, marginBottom: 28 }}>
        {metrics.map((m, i) => (
          <div key={i} style={{ border: "1px solid #eee", borderRadius: 10, padding: 22, textAlign: "center", borderTop: "3px solid #F0B90B" }}>
            <div style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 50, color: "#0A0A0A", lineHeight: 1 }}>
              {m.n}<span style={{ color: "#F0B90B" }}>{m.sym}</span>
            </div>
            <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: 2.5, color: "#aaa", textTransform: "uppercase", marginTop: 4 }}>{m.label}</div>
          </div>
        ))}
      </div>
